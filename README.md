...
audited price pairs, and the four-pool engine comparison re-implemented on
the deposited core as exploratory structural sensitivity (F-029/F-030:
node `fourpool_comparison`; year-30 SOC-loss ratios 0.9-2.2x Century,
median 1.65x, larger in seven of eight regions; the median moves 1.56-1.77
across a clay+silt range of 0.35-0.75).
The claim register that gates every quoted number is `docs/claims.yaml`
(20 claims, 74 checks, document basis v17), run by `make verify`; the
pre-rebuild register is archived under `docs/archive/`.
...
```
make verify        # the gate: 18 analytical test suites + the 33-node build graph
...
| Four-pool / Century year-30 SOC-loss ratio (SC1, exploratory) | **0.9-2.2x**, median **1.65x** (1.56-1.77 across clay+silt 0.35-0.75); larger in 7 of 8 regions; CUE share of the engine difference 6-59 % where defined | Note 2, table 2, Figure S5 |
...
**Four-pool engine.** The microbially-explicit 4-pool scheme is part of the
deposit (`code/model/som_4pool_monthly.py`, `code/model/coupled_4pool.py`;
node `fourpool_comparison`), ported under F-029 from the project's April
line onto the released core (ERA5, realized clearing, stationary spin-up,
production-path calibration). It regenerates Supplementary Note 2's engine
comparison, Supplementary table 2 and Figure S5
(`figures/Figure_S5_fourpool_flux.png`) as EXPLORATORY structural
sensitivity: year-30 SOC-loss ratios 0.9-2.2x Century (median 1.65x,
1.56-1.77 across a clay+silt range of 0.35-0.75), larger in seven of eight
regions; the engines equilibrate at different baseline states (disclosed in
Note 2, e.g. SSA 13.5 vs 9.0 t C/ha), roughly half of equilibrium MAOM
formation arrives by direct DOM sorption, the SSA calibration shortfall is
disclosed (equilibrium yield 1.37 t/ha against the 1.50 target), and every
four-pool parameter is in the parameter ledger. The legacy pre-port figure is kept under
`excluded_legacy_sol/` as a record. Table S4, its Figure S12 curves and
