"""
Coupled Economic-MEMS-Biophysical Model
========================================

Integrates Dale Manning's partial equilibrium economic framework with
Wallenstein's MEMS-based monthly N model (4-pool: POM/DOM/MBC/MAOM +
monthly N availability).

Structural sensitivity companion to coupled_monthly.py. The economic
layer is IDENTICAL; only the biophysical engine differs:
  - Century 3-pool (active/slow/passive) replaced by MEMS 4-pool
  - N mineralization from microbial stoichiometric balance, not pool
    turnover × C:N
  - CUE varies with N availability (downregulates under N stress)
  - MAOM saturation limits mineral-associated C accumulation
  - Priming: fresh C availability modulates MAOM desorption
  - Necromass partitioning governs long-term MAOM formation

The monthly N balance layer (Q10 temperature scaling, moisture
correction, leaching, denitrification, demand-scaled crop uptake)
is shared with the Century version to isolate the effect of SOM
pool structure on coupled model behavior.

Architecture:
    At each annual timestep:
    1. Economic module resolves market equilibrium (fertilizer demand,
       food price, land allocation) given current soil state and price
       shock — IDENTICAL to coupled_monthly.py.
    2. MEMS biophysical module runs one annual pool step + 12-month
       N balance with the economically determined fertilizer rate.
    3. Yield computed from crop N uptake (Mitscherlich + stoichiometric
       cap + water stress).
    4. Residue C input computed from yield; fed to next year's MEMS step
       (lagged, matching standalone MEMS model).
    5. CUE adjusts to N availability state; local elasticities computed
       for next period's economic solve.

Author: Matthew Wallenstein & Dale Manning
"""

import numpy as np
import pandas as pd
from dataclasses import dataclass
from typing import Dict, Tuple

from soil_n_model import (
    SOMPoolParams, CropParams, RegionParams, FeedbackParams,
    get_default_regions,
)

# MEMS model components
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'scripts'))
from monthly_mems_v1 import (
    MEMSPoolParams, mems_annual_step, mems_spinup, mems_dynamic_spinup,
    monthly_n_balance_mems,
    run_model as mems_run_model,
    calibrate_ym as mems_calibrate_ym,
)

# Shared monthly N infrastructure
from monthly_model_v3 import (
    MonthlyClimate, MonthlyNParams, REGIONAL_CLIMATES, FAOSTAT_TARGETS,
    temp_factor, moist_factor,
    growing_months, demand_profile, fert_profile,
    get_regional_bnf,
)

# Re-export economic infrastructure unchanged
from coupled_econ_biophysical import (
    EconParams, REGIONAL_ECON_PARAMS,
    calibrate_price_shock, get_scenario_params, get_supply_constrained_scenarios,
)


# Divisor for converting SOC (t C/ha) to a scaled SOC index used by the
# water-stress feedback.  Derived from 30cm depth, bulk density ~1.0 g/cm³.
# The whc_sensitivity parameter is calibrated against this scale.
SOC_SCALE_DIVISOR = 300 * 0.01  # = 3.0

# ============================================================
# MEMS BIOPHYSICAL ENGINE
# ============================================================

class MEMSBiophysicalEngine:
    """Wraps the MEMS 4-pool model to provide N/yield dynamics for the
    economic model.

    At each annual timestep, accepts externally determined fertilizer rate
    and returns yield, N mineralization, updated MEMS pool state, CUE,
    MAOM saturation, and local elasticities for the economic solver.
    """

    def __init__(self, region: RegionParams, region_key: str = None,
                 crop_params: CropParams = None,
                 feedback_params: FeedbackParams = None,
                 monthly_params: MonthlyNParams = None,
                 mems_params: MEMSPoolParams = None,
                 yield_max_override: float = None):
        self.region = region
        self.region_key = region_key
        self.crop = crop_params or CropParams()
        self.fb = feedback_params or FeedbackParams()
        self.mp = monthly_params or MonthlyNParams()
        self.mems = mems_params or MEMSPoolParams()

        # Climate
        self.climate = REGIONAL_CLIMATES.get(region_key, REGIONAL_CLIMATES['north_america'])

        # MAOM sorption capacity
        clay_silt = getattr(region, 'clay_silt_fraction', 0.55)
        self.qmax = self.mems.qmax_per_claysilt * clay_silt

        # Yield parameters
        self.mit_c = (region.mitscherlich_c_regional
                      if region.mitscherlich_c_regional > 0 else self.crop.mitscherlich_c)

        if yield_max_override is not None:
            self.y_max = yield_max_override
        elif region_key and region_key in FAOSTAT_TARGETS:
            self.y_max = mems_calibrate_ym(region_key, FAOSTAT_TARGETS[region_key],
                                           self.mp, self.mems)
        else:
            self.y_max = region.yield_max_regional
        self.y_floor = region.yield_min_regional if region.yield_min_regional > 0 else 0.0

        # Stoichiometric N cost per tonne grain
        self.n_cost_per_tonne = self.crop.grain_n_fraction * 1000  # 18 kg N / t grain

        # Baseline BNF
        self.bnf_baseline = get_regional_bnf(region_key or 'north_america')

        # Dynamic spinup: iterate to true steady state (eliminates
        # MAOM oversaturation transient from analytical initialization)
        eq = mems_dynamic_spinup(
            region_key or 'north_america',
            p=self.mp, mems_p=self.mems,
            synth_n=region.synth_n_current,
            yield_max_override=self.y_max,
        )
        self.c_pom = eq['c_pom']
        self.c_dom = eq['c_dom']
        self.c_mbc = eq['c_mbc']
        self.c_maom = eq['c_maom']
        self.pom_baseline = eq['pom_baseline']  # priming reference (from analytical init)
        self.soc_initial = eq['soc']

        # Mineral N pool at equilibrium
        self.mineral_n = eq['mineral_n']

        # Baseline residue C input at equilibrium
        self.baseline_c_input = eq['c_input_eq']
        self.lagged_c_input = eq['c_input_eq']

        # Baseline N total for CUE fraction
        self.baseline_n_total = (region.synth_n_current + self.bnf_baseline +
                                 eq['n_min_eq'])

        # Baseline yield and N mineralization from equilibrium
        self.yield_baseline = eq['yield_eq']
        self.n_min_baseline = eq['n_min_eq']

        # Track previous total N for CUE calculation
        self.prev_n_total = self.baseline_n_total

    def _water_stress(self) -> float:
        """Water stress factor (0-1) based on SOC-driven WHC loss."""
        if not self.fb.physical_feedback:
            return 1.0

        soc_current = self.c_pom + self.c_dom + self.c_mbc + self.c_maom
        soc_pct = soc_current / SOC_SCALE_DIVISOR
        soc_pct_init = self.soc_initial / SOC_SCALE_DIVISOR
        delta = max(0, soc_pct_init - soc_pct)
        whc_loss_mm = delta * self.region.whc_sensitivity * self.fb.physical_strength
        total_deficit = self.region.baseline_water_deficit + max(0, whc_loss_mm)
        stress = 1.0 - self.region.water_stress_coeff * total_deficit
        return max(0.3, min(1.0, stress))

    def step(self, fert_applied: float, bnf: float = None) -> Dict:
        """Advance one year with externally determined fertilizer.

        Args:
            fert_applied: kg N/ha/yr of synthetic fertilizer
            bnf: biological N fixation (kg N/ha/yr), defaults to self.bnf_baseline

        Returns:
            dict with yield, N dynamics, MEMS pool state, elasticities, CUE
        """
        if bnf is None:
            bnf = self.bnf_baseline

        # CUE adjusts to N availability
        n_frac = max(0.01, self.prev_n_total / max(self.baseline_n_total, 1.0))

        # Annual MEMS pool step with lagged residue C input
        mems_step = mems_annual_step(
            self.c_pom, self.c_dom, self.c_mbc, self.c_maom,
            c_input=self.lagged_c_input, qmax=self.qmax, mems=self.mems,
            n_available_frac=n_frac, pom_baseline=self.pom_baseline)
        annual_n_min = max(0, mems_step['net_n_mineralized'])

        # Update MEMS pools
        self.c_pom = mems_step['c_pom']
        self.c_dom = mems_step['c_dom']
        self.c_mbc = mems_step['c_mbc']
        self.c_maom = mems_step['c_maom']
        soc_new = self.c_pom + self.c_dom + self.c_mbc + self.c_maom

        # Monthly N balance with MEMS mineralization
        nb = monthly_n_balance_mems(
            annual_n_min, fert_applied, bnf,
            self.region.atm_n_deposition, self.climate,
            self.mineral_n, self.mp)
        self.mineral_n = nb['mineral_n_end']
        n_uptake = nb['uptake']
        n_min_annual = nb['min']

        # Water stress from SOC state
        ws = self._water_stress()

        # Yield from crop N uptake
        y_mit = self.y_max * (1.0 - np.exp(-self.mit_c * n_uptake)) * ws
        y_stoich = n_uptake / self.n_cost_per_tonne if self.n_cost_per_tonne > 0 else y_mit
        y = min(y_mit, y_stoich)
        y = max(self.y_floor, y)

        # Residue C input for NEXT year (lagged)
        hi = self.crop.harvest_index
        rf = (1 - hi) / hi
        rr = self.region.residue_retention
        shoot_c = y * 1000 * 0.45 * rf * rr / 1000
        root_c = y * 1000 * 0.45 * rf * self.region.root_shoot_c_ratio / 1000
        self.lagged_c_input = (shoot_c + root_c) * self.region.cre_regional

        # Update prev_n_total for next step's CUE
        self.prev_n_total = fert_applied + bnf + annual_n_min

        # Compute local elasticities for economic module
        # (identical logic to Century engine)
        eps = 1e-10
        n_up = max(n_uptake, eps)

        if y_stoich < y_mit:
            elasticity_n_total = 1.0
        else:
            exp_term = np.exp(-self.mit_c * n_up)
            denom = max(1.0 - exp_term, eps)
            elasticity_n_total = self.mit_c * n_up * exp_term / denom

        total_n_input = n_min_annual + fert_applied + bnf + self.region.atm_n_deposition
        if total_n_input > eps:
            soil_share = n_min_annual / total_n_input
            fert_share = fert_applied / total_n_input
        else:
            soil_share = 0.5
            fert_share = 0.5

        beta = elasticity_n_total * max(0.0, soil_share)
        gamma = elasticity_n_total * fert_share

        return {
            'yield_tha': y,
            'yield_fraction': y / self.yield_baseline if self.yield_baseline > 0 else 0,
            'n_mineralized': n_min_annual,
            'n_uptake': n_uptake,
            'n_leached': nb['leach'],
            'n_denitrified': nb['den'],
            'n_immobilized': nb['immob'],
            'soc_total': soc_new,
            'soc_fraction': soc_new / self.soc_initial if self.soc_initial > 0 else 1,
            'water_stress': ws,
            'beta': beta,
            'gamma': gamma,
            # MEMS-specific outputs
            'cue': mems_step['cue'],
            'maom_sat': mems_step['maom_sat'],
            'priming': mems_step['priming'],
            'c_pom': self.c_pom,
            'c_dom': self.c_dom,
            'c_mbc': self.c_mbc,
            'c_maom': self.c_maom,
            # Per-step flux decomposition (t C/ha/yr) for Fig 6 flux-attribution.
            # Added 2026-04-14 (matched-mems-2026-04-15 effort).
            'flux_pom_to_dom': mems_step.get('flux_pom_to_dom', 0.0),
            'flux_maom_desorb_to_dom': mems_step.get('flux_maom_desorb_to_dom', 0.0),
            'flux_dom_removed': mems_step.get('flux_dom_removed', 0.0),
            'flux_dom_to_mic': mems_step.get('flux_dom_to_mic', 0.0),
            'flux_dom_to_maom_sorption': mems_step.get('flux_dom_to_maom_sorption', 0.0),
            'flux_mic_assimilated': mems_step.get('flux_mic_assimilated', 0.0),
            'resp_cue': mems_step.get('resp_cue', 0.0),
            'flux_mic_death': mems_step.get('flux_mic_death', 0.0),
            'flux_necro_to_maom': mems_step.get('flux_necro_to_maom', 0.0),
            'flux_necro_to_pom': mems_step.get('flux_necro_to_pom', 0.0),
            'flux_necro_to_dom': mems_step.get('flux_necro_to_dom', 0.0),
            'resp_necro': mems_step.get('resp_necro', 0.0),
            'total_respired': mems_step.get('total_respired', 0.0),
        }


# ============================================================
# COUPLED MODEL
# ============================================================

class CoupledMEMSModel:
    """
    Coupled economic-biophysical model using MEMS 4-pool SOM dynamics.

    Same economic equilibrium framework as CoupledMonthlyModel
    but with MEMSBiophysicalEngine replacing MonthlyBiophysicalEngine.

    At each timestep:
    1. Economic module solves for PY_hat, F_hat, L_hat
    2. MEMS biophysical module runs one annual pool step + 12 monthly
       N balance steps with economically determined F
    3. Yield, SOC, CUE, and elasticities feed back to next period
    """

    def __init__(
        self,
        region: RegionParams,
        econ: EconParams,
        region_key: str = None,
        t_max: float = 100.0,
        dt: float = 1.0,
        yield_max_override: float = None,
        mems_params: MEMSPoolParams = None,
    ):
        self.region = region
        self.econ = econ
        self.region_key = region_key
        self.t_max = t_max
        self.dt = dt

        # Resolve elasticities (same logic as Century coupled model)
        rp = {}
        if region_key and region_key in REGIONAL_ECON_PARAMS:
            rp = REGIONAL_ECON_PARAMS[region_key]

        self.eta = rp.get('eta', econ.eta)
        self.alpha = rp.get('alpha', econ.alpha)
        self.eps_F_PF = rp.get('eps_F_PF', econ.eps_F_PF)
        # When the scenario explicitly zeros eps_F_PY (e.g. S1/S2 disabling
        # the food-price→fertilizer channel), honour that; otherwise use
        # the regional override.
        self.eps_F_PY = econ.eps_F_PY if econ.eps_F_PY == 0.0 else rp.get('eps_F_PY', econ.eps_F_PY)
        self.eps_F_N = econ.eps_F_N

        if econ.eps_LD_PL == 0.0 and econ.eps_LD_PY == 0.0 and econ.eps_LS_PL == 0.0:
            self.eps_LD_PL = 0.0
            self.eps_LD_PY = 0.0
            self.eps_LS_PL = 0.0
        else:
            self.eps_LD_PL = rp.get('eps_LD_PL', econ.eps_LD_PL)
            self.eps_LD_PY = rp.get('eps_LD_PY', econ.eps_LD_PY)
            self.eps_LS_PL = rp.get('eps_LS_PL', econ.eps_LS_PL)

        # Initialize MEMS biophysical engine
        self.bio = MEMSBiophysicalEngine(
            region, region_key=region_key,
            yield_max_override=yield_max_override,
            mems_params=mems_params,
        )

        # Baseline values
        self.F_baseline = region.synth_n_current
        self.L_baseline = region.cropland_mha
        self.Y_baseline = self.bio.yield_baseline
        self.N_min_baseline = self.bio.n_min_baseline

        # Price shock
        self.PF_hat_base = np.log(1 + econ.fert_price_shock)

        # Track log-changes
        self.PF_hat = self.PF_hat_base
        self.PY_hat = 0.0
        self.F_hat = 0.0
        self.L_hat = 0.0
        self.N_hat = 0.0

    def _solve_equilibrium(self, beta: float, gamma: float) -> Tuple[float, float, float]:
        """Solve the simultaneous system for PY_hat, F_hat, L_hat.

        Identical to coupled_monthly.py.
        """
        if abs(self.eps_LS_PL - self.eps_LD_PL) > 1e-10:
            lambda_L = self.eps_LS_PL * self.eps_LD_PY / (self.eps_LS_PL - self.eps_LD_PL)
        else:
            lambda_L = 0.0

        numerator = (beta * self.N_hat +
                     gamma * (self.eps_F_PF * self.PF_hat + self.eps_F_N * self.N_hat))
        denominator = self.eta - self.alpha * lambda_L - gamma * self.eps_F_PY

        if abs(denominator) > 1e-10:
            PY_hat = numerator / denominator
        else:
            PY_hat = 0.0

        F_hat = self.eps_F_PF * self.PF_hat + self.eps_F_PY * PY_hat + self.eps_F_N * self.N_hat
        L_hat = lambda_L * PY_hat

        return PY_hat, F_hat, L_hat

    def run(self) -> pd.DataFrame:
        """Run the coupled simulation."""
        n_steps = int(self.t_max / self.dt) + 1

        results = {
            'year': np.zeros(n_steps),
            'PF_hat': np.zeros(n_steps),
            'PY_hat': np.zeros(n_steps),
            'F_hat': np.zeros(n_steps),
            'L_hat': np.zeros(n_steps),
            'N_hat': np.zeros(n_steps),
            'fert_applied_kgha': np.zeros(n_steps),
            'land_mha': np.zeros(n_steps),
            'food_price_index': np.zeros(n_steps),
            'yield_tha': np.zeros(n_steps),
            'yield_fraction': np.zeros(n_steps),
            'n_mineralized': np.zeros(n_steps),
            'n_uptake': np.zeros(n_steps),
            'n_leached': np.zeros(n_steps),
            'n_denitrified': np.zeros(n_steps),
            'n_immobilized': np.zeros(n_steps),
            'soc_total': np.zeros(n_steps),
            'soc_fraction': np.zeros(n_steps),
            'water_stress': np.zeros(n_steps),
            'beta': np.zeros(n_steps),
            'gamma': np.zeros(n_steps),
            'total_production_index': np.zeros(n_steps),
            'carrying_capacity_fraction': np.zeros(n_steps),
            # MEMS-specific
            'cue': np.zeros(n_steps),
            'maom_sat': np.zeros(n_steps),
            'priming': np.zeros(n_steps),
            'c_pom': np.zeros(n_steps),
            'c_dom': np.zeros(n_steps),
            'c_mbc': np.zeros(n_steps),
            'c_maom': np.zeros(n_steps),
            # Per-step flux decomposition (t C/ha/yr); for matched-mems Fig 6 flux attribution
            'flux_pom_to_dom': np.zeros(n_steps),
            'flux_maom_desorb_to_dom': np.zeros(n_steps),
            'flux_dom_removed': np.zeros(n_steps),
            'flux_dom_to_mic': np.zeros(n_steps),
            'flux_dom_to_maom_sorption': np.zeros(n_steps),
            'flux_mic_assimilated': np.zeros(n_steps),
            'resp_cue': np.zeros(n_steps),
            'flux_mic_death': np.zeros(n_steps),
            'flux_necro_to_maom': np.zeros(n_steps),
            'flux_necro_to_pom': np.zeros(n_steps),
            'flux_necro_to_dom': np.zeros(n_steps),
            'resp_necro': np.zeros(n_steps),
            'total_respired': np.zeros(n_steps),
        }

        # Compute initial elasticities without mutating state
        bnf_base = self.bio.bnf_baseline

        # Get baseline MEMS N mineralization for initial elasticities
        # (use equilibrium pools from dynamic spinup)
        init_step = mems_annual_step(
            self.bio.c_pom, self.bio.c_dom,
            self.bio.c_mbc, self.bio.c_maom,
            c_input=self.bio.baseline_c_input, qmax=self.bio.qmax,
            mems=self.bio.mems, n_available_frac=1.0,
            pom_baseline=self.bio.pom_baseline)
        init_n_min_annual = max(0, init_step['net_n_mineralized'])

        init_nb = monthly_n_balance_mems(
            init_n_min_annual, self.F_baseline, bnf_base,
            self.region.atm_n_deposition, self.bio.climate,
            self.bio.mineral_n, self.bio.mp)
        n_up_init = init_nb['uptake']
        n_min_init = init_nb['min']

        eps = 1e-10
        n_up_safe = max(n_up_init, eps)

        y_mit_init = self.bio.y_max * (1.0 - np.exp(-self.bio.mit_c * n_up_safe))
        y_stoich_init = n_up_safe / self.bio.n_cost_per_tonne

        if y_stoich_init < y_mit_init:
            init_elast = 1.0
        else:
            exp_term = np.exp(-self.bio.mit_c * n_up_safe)
            denom = max(1.0 - exp_term, eps)
            init_elast = self.bio.mit_c * n_up_safe * exp_term / denom

        total_n_input = n_min_init + self.F_baseline + bnf_base + self.region.atm_n_deposition
        soil_share = n_min_init / max(total_n_input, eps)
        fert_share = self.F_baseline / max(total_n_input, eps)
        init_beta = init_elast * max(0.0, soil_share)
        init_gamma = init_elast * fert_share

        for i in range(n_steps):
            t = i * self.dt
            results['year'][i] = t

            if i == 0:
                soc_0 = self.bio.c_pom + self.bio.c_dom + self.bio.c_mbc + self.bio.c_maom
                results['PF_hat'][i] = 0.0
                results['PY_hat'][i] = 0.0
                results['F_hat'][i] = 0.0
                results['L_hat'][i] = 0.0
                results['N_hat'][i] = 0.0
                results['fert_applied_kgha'][i] = self.F_baseline
                results['land_mha'][i] = self.L_baseline
                results['food_price_index'][i] = 1.0
                results['yield_tha'][i] = self.Y_baseline
                results['yield_fraction'][i] = 1.0
                results['n_mineralized'][i] = n_min_init
                results['n_uptake'][i] = n_up_init
                results['n_leached'][i] = init_nb['leach']
                results['n_denitrified'][i] = init_nb['den']
                results['n_immobilized'][i] = init_nb['immob']
                results['soc_total'][i] = soc_0
                results['soc_fraction'][i] = 1.0
                results['water_stress'][i] = self.bio._water_stress()
                results['beta'][i] = init_beta
                results['gamma'][i] = init_gamma
                results['total_production_index'][i] = 1.0
                results['carrying_capacity_fraction'][i] = 1.0
                results['cue'][i] = init_step['cue']
                results['maom_sat'][i] = init_step['maom_sat']
                results['priming'][i] = init_step['priming']
                results['c_pom'][i] = self.bio.c_pom
                results['c_dom'][i] = self.bio.c_dom
                results['c_mbc'][i] = self.bio.c_mbc
                results['c_maom'][i] = self.bio.c_maom
                continue

            # Update N_hat from biophysical state
            current_n_min = results['n_mineralized'][i-1]
            if self.N_min_baseline > 0:
                self.N_hat = np.log(max(current_n_min, 1e-6) / self.N_min_baseline)
            else:
                self.N_hat = 0.0

            # Price recovery
            if (self.econ.price_relaxes_with_recovery and
                    self.econ.fert_capacity_recovery_years > 0 and t > 0):
                recovery_frac = min(1.0, t / self.econ.fert_capacity_recovery_years)
                self.PF_hat = self.PF_hat_base * (1.0 - recovery_frac)
            else:
                self.PF_hat = self.PF_hat_base

            # Get elasticities from previous step
            beta = results['beta'][i-1]
            gamma = results['gamma'][i-1]

            # Solve equilibrium
            PY_hat, F_hat, L_hat = self._solve_equilibrium(beta, gamma)
            self.PY_hat = PY_hat
            self.F_hat = F_hat
            self.L_hat = L_hat

            # Convert to levels
            F_level = self.F_baseline * np.exp(F_hat)
            L_level = self.L_baseline * np.exp(L_hat)
            F_level = max(0.0, F_level)

            # Supply ceiling
            if self.econ.fert_supply_ceiling < 1.0:
                ceiling = self.econ.fert_supply_ceiling
                if self.econ.fert_capacity_recovery_years > 0 and t > 0:
                    recovery_frac = min(1.0, t / self.econ.fert_capacity_recovery_years)
                    ceiling = ceiling + (1.0 - ceiling) * recovery_frac
                total_n_available = self.F_baseline * self.L_baseline * ceiling
                F_max = total_n_available / max(L_level, 1e-6)
                F_level = min(F_level, F_max)

            # Advance biophysical model
            bio_state = self.bio.step(F_level)

            # Total production index
            yield_frac = bio_state['yield_fraction']
            land_frac = L_level / self.L_baseline
            total_prod_index = yield_frac * land_frac

            # Store results
            results['PF_hat'][i] = self.PF_hat
            results['PY_hat'][i] = PY_hat
            results['F_hat'][i] = F_hat
            results['L_hat'][i] = L_hat
            results['N_hat'][i] = self.N_hat
            results['fert_applied_kgha'][i] = F_level
            results['land_mha'][i] = L_level
            results['food_price_index'][i] = np.exp(PY_hat)
            results['yield_tha'][i] = bio_state['yield_tha']
            results['yield_fraction'][i] = yield_frac
            results['n_mineralized'][i] = bio_state['n_mineralized']
            results['n_uptake'][i] = bio_state['n_uptake']
            results['n_leached'][i] = bio_state['n_leached']
            results['n_denitrified'][i] = bio_state['n_denitrified']
            results['n_immobilized'][i] = bio_state['n_immobilized']
            results['soc_total'][i] = bio_state['soc_total']
            results['soc_fraction'][i] = bio_state['soc_fraction']
            results['water_stress'][i] = bio_state['water_stress']
            results['beta'][i] = bio_state['beta']
            results['gamma'][i] = bio_state['gamma']
            results['total_production_index'][i] = total_prod_index
            results['carrying_capacity_fraction'][i] = total_prod_index
            # MEMS-specific
            results['cue'][i] = bio_state['cue']
            results['maom_sat'][i] = bio_state['maom_sat']
            results['priming'][i] = bio_state['priming']
            results['c_pom'][i] = bio_state['c_pom']
            results['c_dom'][i] = bio_state['c_dom']
            results['c_mbc'][i] = bio_state['c_mbc']
            results['c_maom'][i] = bio_state['c_maom']
            # Flux decomposition (matched-mems Fig 6 attribution)
            for fkey in ('flux_pom_to_dom','flux_maom_desorb_to_dom','flux_dom_removed',
                         'flux_dom_to_mic','flux_dom_to_maom_sorption','flux_mic_assimilated',
                         'resp_cue','flux_mic_death','flux_necro_to_maom','flux_necro_to_pom',
                         'flux_necro_to_dom','resp_necro','total_respired'):
                results[fkey][i] = bio_state.get(fkey, 0.0)

        return pd.DataFrame(results)


# ============================================================
# CALIBRATION CACHE
# ============================================================

_YM_CACHE_MEMS = {}

def _mp_cache_key(mp: MonthlyNParams) -> tuple:
    return (mp.q10, mp.t_ref, mp.t_min, mp.moist_opt_lo, mp.moist_opt_hi,
            mp.moist_min, mp.moist_waterlog, mp.leach_coeff, mp.leach_base,
            mp.denitrif_base, mp.denitrif_wet_mult, mp.immob_frac,
            mp.max_uptake_frac, mp.min_n_pool)

def _mems_cache_key(m: MEMSPoolParams) -> tuple:
    return (m.f_pom, m.f_maom, m.f_mbc, m.f_dom,
            m.cn_pom, m.cn_dom, m.cn_mbc, m.cn_maom,
            m.k_pom_to_dom, m.k_maom_desorption, m.k_mbc_turnover,
            m.k_dom_uptake, m.k_dom_sorption,
            m.cue_max, m.cue_min, m.cue_km_n,
            m.f_necro_to_maom, m.f_necro_to_pom, m.f_necro_to_dom,
            m.qmax_per_claysilt, m.priming_sensitivity)

def get_calibrated_ym_mems(region_key: str, mp: MonthlyNParams = None,
                           mems: MEMSPoolParams = None) -> float:
    if mp is None:
        mp = MonthlyNParams()
    if mems is None:
        mems = MEMSPoolParams()
    cache_key = (region_key, _mp_cache_key(mp), _mems_cache_key(mems))
    if cache_key not in _YM_CACHE_MEMS:
        target = FAOSTAT_TARGETS[region_key]
        _YM_CACHE_MEMS[cache_key] = mems_calibrate_ym(region_key, target, mp, mems)
    return _YM_CACHE_MEMS[cache_key]

def clear_ym_cache_mems():
    _YM_CACHE_MEMS.clear()


# ============================================================
# SMOKE TEST
# ============================================================

if __name__ == '__main__':
    print('=' * 70)
    print('COUPLED MEMS MODEL — Smoke Test')
    print('=' * 70)

    regions = get_default_regions()
    mp = MonthlyNParams()
    mems = MEMSPoolParams()

    # Calibrate all regions
    print('\nCalibrating yield_max for each region (MEMS)...')
    ym_cal = {}
    for rk in REGIONAL_CLIMATES:
        ym_cal[rk] = get_calibrated_ym_mems(rk, mp, mems)
        print(f'  {rk}: ym = {ym_cal[rk]:.3f}')

    # Run S3 for all regions
    scenarios = get_scenario_params()
    s3 = scenarios['S3']
    shock = calibrate_price_shock(0.20)
    print(f'\nCalibrated price shock: {shock:.4f} ({shock*100:.1f}%)')

    print(f'\n{"Region":<25} {"Y_base":>7} {"Y_yr10":>7} {"Loss%":>7} '
          f'{"SOC_0":>7} {"SOC_10":>7} {"CUE_10":>7} {"MAOM%":>7}')
    print('-' * 100)

    for rk, r in regions.items():
        ym = ym_cal[rk]
        model = CoupledMEMSModel(
            region=r, econ=s3, region_key=rk, t_max=30.0,
            yield_max_override=ym, mems_params=mems,
        )
        df = model.run()
        yr0 = df[df['year'] == 0].iloc[0]
        yr10 = df[df['year'] == 10].iloc[0]
        loss = (1 - yr10['yield_fraction']) * 100
        print(f'  {r.name:<23} {yr0["yield_tha"]:>7.2f} {yr10["yield_tha"]:>7.2f} '
              f'{loss:>6.1f}% {yr0["soc_total"]:>7.1f} {yr10["soc_total"]:>7.1f} '
              f'{yr10["cue"]:>7.3f} {yr10["maom_sat"]*100:>6.1f}%')

    # --- Compare Century vs MEMS for one region ---
    print('\n' + '=' * 70)
    print('CENTURY vs MEMS — North America, S3, 30yr')
    print('=' * 70)
    from coupled_monthly import CoupledMonthlyModel, get_calibrated_ym

    rk = 'north_america'
    r = regions[rk]

    ym_c = get_calibrated_ym(rk, mp)
    ym_m = ym_cal[rk]

    model_c = CoupledMonthlyModel(region=r, econ=s3, region_key=rk,
                                   t_max=30.0, yield_max_override=ym_c)
    model_m = CoupledMEMSModel(region=r, econ=s3, region_key=rk,
                                t_max=30.0, yield_max_override=ym_m,
                                mems_params=mems)
    df_c = model_c.run()
    df_m = model_m.run()

    print(f'\n{"Year":>5} {"C_yield":>8} {"M_yield":>8} {"C_SOC%":>7} {"M_SOC%":>7} '
          f'{"C_Nmin":>7} {"M_Nmin":>7} {"M_CUE":>6}')
    print('-' * 65)
    for yr in [0, 1, 5, 10, 20, 30]:
        rc = df_c[df_c['year'] == yr].iloc[0]
        rm = df_m[df_m['year'] == yr].iloc[0]
        print(f'  {yr:>3d}  {rc["yield_tha"]:>8.2f} {rm["yield_tha"]:>8.2f} '
              f'{rc["soc_fraction"]*100:>6.1f}% {rm["soc_fraction"]*100:>6.1f}% '
              f'{rc["n_mineralized"]:>7.1f} {rm["n_mineralized"]:>7.1f} '
              f'{rm["cue"]:>6.3f}')

    print('\nDone.')
