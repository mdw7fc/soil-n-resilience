...
- `excluded_legacy_sol/Figure_S5_flux_decomposition_legacy_sol.png` — legacy
  exploratory figure from the microbially-explicit 4-pool scheme, whose code was never
  deposited. **Figure S5 was withdrawn from the paper**; Supplementary Note paragraph 200
  records the removal, and it is the only surviving reference to S5 in either document.
  The figure is kept here as a record of what was withdrawn. **No generator is owed:**
  `make_figure_s5.py` was carried as a debt through F-009 and §7 of the assurance
  handoff, and D3 retired it rather than writing a generator for a figure the paper does
  not contain.
