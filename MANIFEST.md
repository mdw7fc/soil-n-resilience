...
register that gates every quoted number is `docs/claims.yaml` (20 claims, 74
...
- `docs/claims.yaml` — the claim register: 20 claims, 74 checks, document basis v17
- `docs/archive/CLAIM_REGISTER_sol.csv` / `.md` — pre-F-025 register, archived; values are historical
- `PARAMETER_LEDGER_sol.csv` / `.md` — 604 semantic live-parameter entries (F-030 added the 22 four-pool structural parameters)
- `NUMERIC_LITERAL_AUDIT_sol.csv` — 2,470 audited source-line numeric entries
...
- `climate_comparison.py` — expert vs ERA5 climate robustness (deposit diagnostic; there is
  ...
  Writes `outputs/climate_swap_comparison.csv` and `results/climate_swap_stats.txt`; the
  two headline numbers are deposited rather than printed, because the 0.74 pp this file
  carried for three generations reached it by transcription from a console (F-009)
...
- `climate_swap_comparison.csv` — expert vs ERA5 yr1/yr10 losses (max year-10 shift
  **0.70 pp**, Spearman rank rho **0.98**; read from `results/climate_swap_stats.txt`,
  not transcribed)
...
- `crop_response_calibration_table.csv` — regional crop-response calibration table; this
  is the file Supplementary Table S4 is transcribed from. Written by
  `make_table_s4_sol.py` since D3; before that no script in the deposit wrote it and the
  frozen copy was pre-recalibration
- `benchmarks/` — official 2022 hindcast observations, Broadbalk extraction and caveat table

`crop_response_calibration_table.csv`, `figS12_curves.json` and
`outputs/Table_S4_calibration_sol.csv` are the three outputs of
`code/repro/make_table_s4_sol.py`, including the live simulation-calibrated
`y_max` and simulated no-synthetic-N yield. This was true of the second and
third from the v14 deposit and stated of the first without being true of it
until D3.
...
- `excluded_legacy_sol/Figure_S5_flux_decomposition_legacy_sol.png` — the pre-F-029
  four-pool figure, kept as a record; the live Figure S5 is now generated by
  `code/repro/run_4pool_comparison.py` (`figures/Figure_S5_fourpool_flux.png`) from the
  deposited engine (`code/model/som_4pool_monthly.py`, `code/model/coupled_4pool.py`)
...
- `results/climate_swap_stats.txt` — the two headline numbers from the above, deposited
