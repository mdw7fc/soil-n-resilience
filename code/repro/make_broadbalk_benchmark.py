...
SOC_CSV = ROOT_DIR / 'code' / 'model' / 'data' / 'benchmark_broadbalk' / 'soc_trajectories_broadbalk.csv'
...
def main():
    df = load_soc()
    table_out = ROOT_DIR / "data" / "benchmarks" / "broadbalk_yield_benchmark_sol.csv"
    YIELD.assign(
        observed_low=YIELD["treatment"].map(lambda x: OBS_RANGE[x][0]),
        observed_high=YIELD["treatment"].map(lambda x: OBS_RANGE[x][1]),
    ...
    png = FIG_DIR / 'Figure_S2_broadbalk_benchmark.png'
    pdf = FIG_DIR / 'Figure_S2_broadbalk_benchmark.pdf'
