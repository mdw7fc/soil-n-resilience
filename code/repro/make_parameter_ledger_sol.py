    ...
    for cls, group, scope, location in classes:
        ...
        for field in fields(cls):
            ...
            add(
                ...
                UNITS.get(name, "fraction or elasticity"), "primitive",
                status, SOURCES.get(name, "documented inline and in SI"),
                f"{location}.{name}",
                prior_names.get(name, "fixed; not varied in submitted analyses"),
                "central path" if scope.startswith("canonical") else "fallback",
            )
