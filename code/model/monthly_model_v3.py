...
def monthly_n_balance(
    ...
    `som_params` (optional) overrides the default SOMPoolParams used for
    pool-specific decay rates (k_active, k_slow, k_passive). When not
    provided, defaults to SOMPoolParams() — preserves prior behaviour.
    Used by Monte Carlo ensembles to propagate perturbed SOM kinetics.
    """
    som = som_params if som_params is not None else SOMPoolParams()
...
def century_dynamic_spinup(
    ...
    region = region_override if region_override is not None else regions[region_key]
    ...
    # Optional `som_params` and `crop_params` overrides allow Monte Carlo
    # ensembles to propagate perturbed SOM kinetics and crop traits.
    som = som_params if som_params is not None else som_params_for_region(region_key)
    crop = crop_params if crop_params is not None else CropParams()
    ...
    for yr in range(n_spinup):
        # Monthly N balance — pass perturbed som so MC k_slow propagates.
