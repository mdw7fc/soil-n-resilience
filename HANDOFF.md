...
repair is to the document. `docs/claims.yaml` is the claim register (20 claims,
74 checks); `make verify` runs it and everything else.

## State of the model

The model is green: 18 suites and the 33-node build graph, exit 0, nothing
...
found); the claim register reads 74/74 AGREES on document_basis v17. The
...
5. (retired by F-029) The 4-pool re-port is done: som_4pool_monthly.py +
   coupled_4pool.py + run_4pool_comparison.py (node fourpool_comparison,
   includes the CUE decomposition). Still open from the original plan: the
   F-004 planting-month registration (the crop calendar is now documented in
   the SI and cited to Sacks et al. 2010, but the months remain unregistered
   literals in monthly_model_v3).
