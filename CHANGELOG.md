...
- The four-pool engine comparison is re-implemented on the deposited core
  as exploratory structural sensitivity (F-029/F-030): year-30 SOC-loss
  ratios 0.9-2.2x Century (median 1.65x; 1.56-1.77 across clay+silt
  0.35-0.75), larger in seven of eight regions; the pathway description is
  corrected (roughly half of equilibrium MAOM formation by direct DOM
  sorption), baseline-state differences between the engines are disclosed,
  all four-pool parameters enter the ledger, and Figure S5 reports the
  disruption-attributable carbon budget against a no-shock control.
  Supplementary Note 2, table 2 and Figure S5 regenerate from the deposit
  (the submitted 0.9-2.5x family, computed on a non-deposited engine, is
  superseded). F-031 bounds the mechanism attribution: the fixed-CUE
  analysis assigns 6-59% of the positive 4-pool-Century loss difference to
  CUE downregulation, and the remainder is not partitioned — the larger
  losses arise from the combined effects of the scheme's alternative
  stabilization, respiration, desorption, and baseline-state dynamics.
- The Monte Carlo ensemble was regenerated through the build graph (1,000
  draws, seeded, ERA5): buffering P = 1.0 in every region.
- The claim register moved to `docs/claims.yaml` (20 claims, 74 checks,
...
Ledger: FINDINGS.md F-018 through F-031. Release documents:
